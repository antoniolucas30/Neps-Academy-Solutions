...
int main(){
  ...
  std::cout << mergeSort(vec, 0, (num - 1)) << std::endl;
...
long mergeFcn(long* vec, long leftIndex, long middleIndex, long rightIndex){
  ...
  long* temporaryArray = new long[rightIndex - leftIndex + 1], countInversions = 0, changeLeft = leftIndex, changeRight = middleIndex + 1, indexTemporary = -1;

  while((changeLeft <= middleIndex) && (changeRight <= rightIndex)){

    if(vec[changeLeft] <= vec[changeRight]){

      temporaryArray[++indexTemporary] = vec[changeLeft];
      changeLeft++;
      ...
      temporaryArray[++indexTemporary] = vec[changeRight];
      changeRight++;
  ...
  while(changeLeft <= middleIndex)
    temporaryArray[++indexTemporary] = vec[changeLeft], changeLeft++;

  while(changeRight <= rightIndex)
    temporaryArray[++indexTemporary] = vec[changeRight], changeRight++;
